module "container_apps" {
  ...
  container_app_environment_name = can(each.value.container_app_environment_key) ? try(local.combined_objects_container_app_environments['achilles_development_workload']['achilles_dev_re1'].name,can(local.combined_objects_container_app_environments['achilles_development_workload'],'cannot_find_app_env')) : "Consumption"
