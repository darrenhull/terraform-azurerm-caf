module "container_apps" {
  ...
  container_app_environment_name = can(each.value.container_app_environment_key) ? try(local.combined_objects_container_app_environments[try(each.value.lz_key, local.client_config.landingzone_key)][each.value.container_app_environment_key].name,"Consumption2") : "Consumption2"
